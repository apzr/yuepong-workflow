...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    public ObjectNode convertToJson(BpmnModel model) {
        ...
        if ((model.getPools().size() > 0) && poolHasDI) {
            ...
            // 扩展bpmn解析器
            processFlowElements(model.getMainProcess(), model, shapesArrayNode, null, null, 0.0, 0.0);
            processMessageFlows(model, shapesArrayNode);
        }
    ...
    @Override
    public void processFlowElements(FlowElementsContainer container, BpmnModel bpmnModel, ArrayNode shapesArrayNode, Map<String, ModelInfo> map, Map<String, ModelInfo> map1, double subProcessX, double subProcessY) {
        for (FlowElement flowElement : container.getFlowElements()) {
            processFlowElement(flowElement, container, bpmnModel, shapesArrayNode, subProcessX, subProcessY);
        }

        processArtifacts(container, bpmnModel, shapesArrayNode, subProcessX, subProcessY);
    }

    @Override
    public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement, Map<String, JsonNode> shapeMap, Map<String, String> formKeyMap, Map<String, String> decisionTableMap, BpmnModel bpmnModel) {
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
            ...
            if (STENCIL_POOL.equals(stencilId)) {
                ...
                for (JsonNode laneNode : laneArrayNode) {
                    ...
                    if (STENCIL_LANE.equals(laneStencilId)) {
                        ...
                        processJsonElements(laneNode.get(EDITOR_CHILD_SHAPES), modelNode, lane, shapeMap, null, null, bpmnModel);
        ...
        if (!nonEmptyPoolFound) {
            ...
            processJsonElements(shapesArrayNode, modelNode, process, shapeMap, null, null, bpmnModel);
        } else {
